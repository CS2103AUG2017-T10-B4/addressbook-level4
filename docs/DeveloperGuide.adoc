...
By: `Team SE-EDU`      Since: `Sept-2017`      Licence: `MIT`

== Introduction

This developer guide is designed for developers interested in understanding and developing new features and enhancements for the YellowPages address book application. This guide is a reference manual for developers looking to:

 * Enhance the current YellowPages application for specific operating requirements.
 * Extend the functionality of the current YellowPages application.
...
+
[NOTE]
Having any versions of Java 8 is not enough. +
This app will not work with older versions of Java 8.
+
...
. link:#testing[Run the tests] and ensure all tests pass.
...
1. Familiarise with the overall design by reading the link:#architecture[Architecture] section.
...
* Initializing the components in the correct sequence, and connects them up with each other when launching the app.
...
The rest of the App consists of four aditional components.

* link:#ui-component[*`UI`*] : The User Interface of the App.
* link:#logic-component[*`Logic`*] : The command executor of the app.
* link:#model-component[*`Model`*] : The memory component of the app's data.
* link:#storage-component[*`Storage`*] : The component that handles reading and writing of data to the hard disk for the app.
...
For example, the `Logic` component (figure 3.1.2) defines it's API in the `Logic.java` interface and exposes its functionality using the `LogicManager.java` class.
...
[NOTE]
Note how the event is propagated through the `EventsCenter` to the `Storage` and `UI` without `Model` being coupled to either of them. This shows how the Event Driven approach helps us to reduce direct coupling between components.
...
* will execute user commands using the `Logic` component.
* will bind itself to some data in the `Model` so that the UI can auto-update when data in the `Model` changes.
* will respond to events raised from various parts of the App and update the UI accordingly.
...
Adding multiple contacts is used so as to allow the user to add multiple contacts with the user of a single command, saving time and without the need to manually enter each entry.
`AddMultipleByTsvCommand` extends `UndoableCommand` so it can be undone using `undo` function. By keying the path to the file containing the contacts, the command can parse values of each contact at every line and add them to the address book.

A valid file must satisfy the following conditions:

* Must be a tsv, or in tab separated value form, that is each attribute of a contact (`name`, `phone`,...) is separated by a tab
* The first line must contains the following values, *in this specific order*, corresponding to contact attributes, separated by tab: `name`   `phone` `email` `address`   `tag`

A valid format can be as follow. It is highly advisable that user enter the data in Excel and export as `Text (Tab delimited)`:
...
`AddMultipleByTsvCommand` can throw the following exception:

 * `NoSuchElementException`: Just as `AddCommand`, this happens when `name` or `phone` or `email` is missing, and that specific entry will not be added.
 * `IOException`: This happens when the system cannot find the file specified by users, and no entry will be added.
 * `DuplicatePersonException`: This happens when any entry has already been added, and that specific entry will not be added.
...
Users specify the path to their list of contact as the only argument of the command. `AddressBookParser` parses this argument and create a new instance of `AddMultipleByTsvCommandParser` with the given argument as `contactTsvFilePath`. `AddMultipleByTsvCommandParser` then creates a new instance of `ContactTsvReader` with the given `contactTsvFilePath` and calls `readContactFromFile()` to start parsing.
[NOTE]
`readContactFromFile` throws `NoSuchElementException` and `IOException`. Refer to the above for details.

After `readContactFromFile` is finished, `AddMultipleByTsvCommandParser` calls `getToAddPeople()` for `toAddPeople` (a list of people to add) and `getFailedEntries` for failedEntries (a list of integer of entries that were failed to add due to errors). `AddMultipleByTsvCommandParser` creates a new instance of `addMultipleByTsvCommand` given `toAddPeople` and `failedEntries`. Within this instance, `addPerson` will be called for each person in `toAddPeople`. After this, `addMultipleByTsvCommand` creates a new instance of `CommandResult` and sends the `result` back to `LogicManager`.
...
Delete multiple contacts extends from undoable commands, and it can be undone using the undo function. By keying in the command word `deleteMul` along with `indexes`, the command word will be detected in the `AddressBookParser` class where the arguments (indexes in this case) will be passed over to the `DeleteMultipleCommandParser` class.
...
We use the `java.util.logging` package for logging. The `LogsCenter` class is used to manage the logging levels and logging destinations.
...
* `SEVERE` : Critical problems detected which may cause the termination of the application.
* `WARNING` : Proceed with caution.
* `INFO` : Information showing noteworthy actions by the App.
* `FINE` : Details that  are not usually noteworthy but may be useful in debugging (e.g. printing the actual list instead of just its size).
...
[NOTE]
We chose asciidoc over Markdown because asciidoc provides more flexibility in formatting compared to Markdown.
...
.. _Unit tests_ that test the individual components. These are found in the `seedu.address.ui` package.
...
|`* * *` |user |add remarks to a person | store additional information about that person
...
.  The system should have an easy to follow user guide
.  The system should be able to keep all private tagged information confidential.
.  The system should open the help page when user enters an invalid entry.
.  The system should be easy to maintain in event of malfunction.
