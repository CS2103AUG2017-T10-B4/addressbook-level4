...
YellowPages is a free-to-use enhanced address book application which you can use to keep track of all your contact details. The application comes in-built with multiple new features not found in regular address books which creates a more user-friendly experience for all.
...
=== Locating person by number: `findnum` (Since v1.2)
...
=== Add multiple contacts at once : `addMulTsv` (Since v1.2)

Add multiple contacts to addressBook at once. +
Input format: `addMulTsv TSV_PATH`

****
* Adds multiple contacts in a text file at the specified `tsv_path`
* An acceptable file must be exported from a conventional Excel file in `Text (Tab delimited)` file type
* The first line must contains the following values, in this specific order, separated by tab: `name` `phone` `email` `address` `tag`
* The following lines must contains the columns corresponding to the first line
****

A valid format can be as follow
...
Output format: `NUMBER OF SUCCESSFULLY ADDED PEOPLE` new person (people) added, `NUMBER OF DUPLICATED PEOPLE`
new person (people) duplicated, `NUMBER OF UNSUCCESSFULLY ADDED PEOPLE` entry (entries) failed:
`LIST OF INDICES THAT WERE UNSUCCESSFULLY ADDED`

****
* People which already exist in the address book will not be added again
* People which miss one or more required fields will not be added
****
...
=== Recently deleted contacts : `recentlyDel` (Since in v1.2)

Retrieves a list of recently deleted contacts for the past 30 contacts. +
Format: `recentlyDel`
